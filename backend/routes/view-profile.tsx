...
// CONSTANTS
// ─────────────────────────────────────────────────────────────────────────────
const BASE_URL = "http://172.17.24.135:8030/api"; // Your server IP
    ...
    const fetchWorkerProfile = async () => {
      try {
        ...
        if (response.ok) {
          ...
          Alert.alert(
            "Error",
            data.message || "Failed to load worker profile.",
          );
          router.back();
        }
      } catch (error) {
        ...
        <View style={styles.section}>
          ...
          {!worker.reviews || worker.reviews.length === 0 ? (
            <Text style={styles.emptyText}>
              No reviews yet. Be the first to hire them!
            </Text>
          ) : (
      ...
      <View style={styles.bottomBar}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            Alert.alert(
              "Confirm Hire",
              `Are you sure you want to hire ${worker.name}?`,
            );
          }}
        >
