...
type ApplicationItem = {
  ...
  source?: "direct" | "referral";
  jobId?: string | { _id: string };
  description?: string; // Added to support referral notes
  referralDescription?: string; // Added fallback just in case
};
...
export default function ApplicationListScreen() {
  ...
  const renderItem = ({ item }: { item: ListRow }) => {
    ...
    if (item.type === "application") {
      ...
      const workerPhone = getWorkerPhone(app);
      const workerSkills = getWorkerSkills(app);
      const isReferred = app.source === "referral";
      const appDesc = app.description || app.referralDescription;
      ...
      return (
        <View style={styles.card}>
          ...
          >
            <View style={styles.titleRow}>
              ...
              {isReferred && (
                <View style={styles.referredTag}>
                  <Text style={styles.referredTagText}>Referred Worker</Text>
                </View>
              )}
            </View>
            ...
            {isReferred && appDesc ? (
              <View style={styles.descriptionContainer}>
                <Text style={styles.descriptionLabel}>Referral Note:</Text>
                <Text style={styles.descriptionText}>{appDesc}</Text>
              </View>
            ) : null}
          ...
          <View style={styles.actionRow}>
            ...
            {/* ONLY show chat if it's NOT a referred worker */}
            {!isReferred && (
              <TouchableOpacity
                style={styles.chatBtn}
                onPress={() => handleChat(app)}
    ...
    const referral = item.data;
    const refDesc = referral.description || referral.referralDescription;
    ...
    return (
      <View style={styles.referralCard}>
        ...
        {refDesc ? (
          <View style={styles.descriptionContainer}>
            <Text style={styles.descriptionLabel}>Referral Note:</Text>
            <Text style={styles.descriptionText}>{refDesc}</Text>
          </View>
        ) : null}
      </View>
    );
